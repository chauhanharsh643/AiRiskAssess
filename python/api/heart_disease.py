...
heart_disease_route = Blueprint("heart_disease_route", __name__)

@heart_disease_route.route('/api/heart_disease', methods=['POST'])
def get_details():
    data = request.json
    
    default_values = {
        'age': 50, 'cp': 0, 'trestbps': 120, 'chol': 200, 'fbs': 100,
        'restecg': 0, 'thalach': 150, 'exang': 0, 'oldpeak': 0, 'slope': 0,
        'ca': 0, 'thal': 2
    }
